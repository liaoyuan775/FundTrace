...
from ..ingestion.dedup import canonical_records
...
def count_rapid_transfers(
    incoming: list[TransactionRecord], outgoing: list[TransactionRecord]
) -> int:
    incoming = sorted(incoming, key=lambda t: t.transaction_time)
    outgoing = sorted(outgoing, key=lambda t: t.transaction_time)
    count = 0
    left = 0
    for target in outgoing:
        while (
            left < len(incoming)
            and (target.transaction_time - incoming[left].transaction_time).total_seconds()
            > 1800
        ):
            left += 1
        index = left
...
def has_split_transfer(outgoing: list[TransactionRecord]) -> bool:
    ...
    for right, target in enumerate(outgoing):
        while (
            target.transaction_time - outgoing[left].transaction_time
        ).total_seconds() > 3600:
            left += 1
        if right - left + 1 >= 3:
            return True
...
def score_account_risk(
    ...
    if rapid_count:
        factors.append(
            _risk_factor("rapid_transfer", "快速转出", 20, f"入账后30分钟内转出 {rapid_count} 组")
        )
    ...
    if pass_ratio >= 0.6:
        factors.append(
            _risk_factor("pass_through", "高资金穿透率", 25, f"流出/流入穿透率 {pass_ratio:.0%}")
        )
    elif pass_ratio >= 0.3:
        factors.append(
            _risk_factor("pass_through", "中等资金穿透率", 15, f"流出/流入穿透率 {pass_ratio:.0%}")
        )
    counterparties = {t.payer_account for t in incoming} | {t.payee_account for t in outgoing}
    if len(counterparties) >= 4:
        factors.append(
            _risk_factor("multiple_counterparties", "多收多转", 10, f"关联对手账户 {len(counterparties)} 个")
        )
    if has_split_transfer(outgoing):
        factors.append(
            _risk_factor("split_transfer", "短时分拆", 20, "60分钟内向至少3个下游转出")
        )
    return_records = [t for t in related if "回流" in t.summary]
    if return_records:
        factors.append(
            _risk_factor("return_flow", "资金回流", 15, f"发现回流流水 {len(return_records)} 笔")
        )
    risky_records = [
        t for t in related
        if any(keyword in f"{t.summary} {t.channel}" for keyword in RISK_KEYWORDS)
    ]
    if risky_records:
        factors.append(
            _risk_factor("risky_signal", "风险语义命中", 10, f"摘要或渠道命中风险词 {len(risky_records)} 笔")
        )
...
def build_graph(
    ...
    filtered = [
        ...
        and (not query or query in " ".join([t.serial_number, t.payer_account, t.payer_name, t.payee_account, t.payee_name, t.summary]).lower())
        and (direction == "all" or (direction == "return" and "回流" in t.summary) or (direction == "forward" and "回流" not in t.summary))
        and (not channel or t.channel == channel)
        and (not bank or bank in t.payer_bank or bank in t.payee_bank)
        and (not region or t.region == region)
        and (not date_from or t.transaction_time.date().isoformat() >= date_from)
        and (not date_to or t.transaction_time.date().isoformat() <= date_to)
    ]
    nodes = {}
    edge_map = {}
    related_by_account = defaultdict(list)
    incoming_records = defaultdict(list)
    ...
    risk_summary = {
        ...
        "level": highest["risk_level"] if highest else "低风险",
        "account_id": highest["id"] if highest else None,
        "account_label": highest["label"] if highest else "无",
...
def attribute_mixed_funds(transactions: list[TransactionRecord], source_account: str, victim_amount: float, preexisting_balance: float) -> dict[str, AttributionResult]:
    ...
    remaining = victim_amount
    fifo = []
    for t in outgoing:
        value = min(t.amount, remaining)
        fifo.append(value)
        remaining -= value
    original = preexisting_balance
    conservative = []
    remaining = victim_amount
    for t in outgoing:
        ordinary = min(original, t.amount)
        original -= ordinary
        value = min(max(0, t.amount - ordinary), remaining)
        conservative.append(value)
        remaining -= value
    total_out = sum(t.amount for t in outgoing)
    ratio = victim_amount / (victim_amount + preexisting_balance) if victim_amount + preexisting_balance else 0
    proportional = []
    remaining = victim_amount
    for t in outgoing:
        value = min(t.amount * ratio, remaining)
...
def shortest_path(records: list[TransactionRecord], start: str, end: str) -> list[str]:
    ...
    for t in records:
        graph[t.payer_account].add(t.payee_account)
...
def trace_network(records: list[TransactionRecord], start: str, end: str = "", hops: int = 3) -> dict:
    ...
    for t in records:
        forward[t.payer_account].add(t.payee_account)
        reverse[t.payee_account].add(t.payer_account)
    ...
    return {
        ...
        "shortest_path": shortest_path(records, start, end) if end else [],
        "cycles": cycles[:50],
    }
